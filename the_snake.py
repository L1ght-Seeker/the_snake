...
# Установка размеров экрана и шагов сетки
...
class GameObject:
    ...
    def __init__(self):
        ...
        self.position = []      # Список координат сегментов
        self.body_color = None  # Цвет тела объекта
...
class Apple(GameObject):
    """Класс представляет яблоко на игровом поле."""

    def __init__(self):
        """Инициализирует яблоко и задаёт случайную позицию."""
        super().__init__()
        self.randomize_position()      # Случайная позиция
...
class Snake(GameObject):
    """Класс описывает змею и её поведение на игровом поле."""

    def __init__(self):
        """Инициализирует змею и устанавливает начальные параметры."""
        super().__init__()
        self.body_color = SNAKE_COLOR  # Цвет змеи
    ...
    def reset(self):
        """Сбрасывает состояние змеи в первоначальное."""
        self.positions = [(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2)]
        self.direction = RIGHT
        self.score = 0
    ...
    def move(self):
        ...
        if self.positions.count(self.get_head_position()) > 1:
            raise Exception("Оу нееееет! Вы столкнулись сами с собой :(")
    ...
    def update_direction(self, new_dir):
        """
        Устанавливает новое направление, если оно
        не является противоположным текущему направлению.

        :param new_dir: Новое направление (одно из UP, DOWN, LEFT, RIGHT)
        """
        opposite_dirs = {
            UP: DOWN,
            DOWN: UP,
...
def handle_keys(snake):
    """Обрабатывает события клавиатуры для управления змеёй."""
...
# P.S
# Желаю хорошего дня..ну или что там сейчас у вас))
